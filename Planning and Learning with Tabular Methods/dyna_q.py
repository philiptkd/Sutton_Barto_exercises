...
def get_steps(n, env):
    ...
    while episode < episodes:
        _,_,_,_,done,_,_,_ = next(step_gen)
        step += 1
        if done:
            steps[episode] = step
            step = 0
...
def dyna_q(n, env, plus, Q=None, model=None, action_hist=None):
    ...
    while True:
        ...
        action, reward, next_state_idx, done = step(state_idx, action_hist, env, Q)

        # add exploration bonus for dyna-q+
        if plus:
            tau = action_hist[state_idx].count(action)
            reward += kappa*np.sqrt(tau)
        ...
        q_plan(n, action_hist, env, model, Q)
...
def q_plan(n, action_hist, env, model, Q):
...
def step(state_idx, action_hist, env, Q):
